...
async function main() {
  ...
  try {
    ...
    const env = { STATIC_ROOT: DIST, REQUIRE_DEPLOY_ARTIFACT: '1', BASE_URL: server.url,
      HOME_CHECK_URL: server.url, NAV_CHECK_URL: server.url, NAV_STABILITY_URL: server.url, MOBILE_CHECK_URL: server.url,
    ...
    for (const browser of browsers) {
      ...
      for (const [name, script] of [['nav', 'nav-overlay-check.js'], ['nav-stability', 'navigation-stability-check.js'], ['gallery-release', 'gallery-release-check.js'], ['artifact', 'artifact-browser-check.js']]) {
