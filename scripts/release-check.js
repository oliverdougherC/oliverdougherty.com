...
async function main() {
  ...
  try {
    ...
    const env = { STATIC_ROOT: DIST, REQUIRE_DEPLOY_ARTIFACT: '1', BASE_URL: server.url,
      ...
      GALLERY_CHECK_URL: server.url, GALLERY_HEADING_URL: server.url, UTILITIES_CHECK_URL: server.url, STRESS_CHECK_URL: server.url };
    ...
    for (const browser of browsers) {
      ...
      for (const [name, script] of [['nav', 'nav-overlay-check.js'], ['gallery-release', 'gallery-release-check.js'], ['gallery-heading', 'gallery-heading-check.js'], ['artifact', 'artifact-browser-check.js']]) {
